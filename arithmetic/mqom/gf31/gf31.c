...
const uint8_t gf31_inverse_tab[31] = {0, 1, 16, 21, 8, 25, 26, 9, 4, 7, 28, 17, 13, 12, 20, 29, 2, 11, 19, 18, 14, 3, 24, 27, 22, 5, 6, 23, 10, 15, 30};
...
// vz[] = vz[] * y + vx[]
void gf31_mul_and_add_tab(uint8_t* vz, uint8_t y, const uint8_t* vx, uint32_t size) {
    uint16_t res;
    for(uint32_t i=0; i<size; i++) {
        res = vz[i]*y + vx[i];
        vz[i] = (uint8_t)_gf31_reduce16(res);
...
void gf31_random_elements_x4(uint8_t* const* points, uint32_t nb_points, samplable_x4_t* entropy) {
    uint32_t buffer_size = 5*((nb_points+(nb_points>>4)+7)>>3);
    uint8_t* buffer_mem = (uint8_t*)malloc(4*buffer_size);
    uint8_t* buffer[4] = {
