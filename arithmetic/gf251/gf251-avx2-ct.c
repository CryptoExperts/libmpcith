/************************************************
 *    File from the SDitH Submission Package.   *
...
/// Performs "z[16] += vx[m] * my[m][16]"
/// parallelizaion over m
#define with_offset(p,i,x) ((__m128i const *) (((uint8_t const*) p)+i*x))
void p251_vec_mat16cols_muladd_b16_avx2_ct_offset(void *vz, uint8_t const *vx,
  ...
  for (size_t i = 0; i < mm1; i+=2) {
    ...
    const __m256i VA_0 = _mm256_set1_epi16(vx[i]);
    const __m256i VA_1 = _mm256_set1_epi16(vx[i+1]);
    const __m256i VB_0 = _mm256_cvtepu8_epi16(_mm_loadu_si128(with_offset(myy,i,offset)));
    const __m256i VB_1 = _mm256_cvtepu8_epi16(_mm_loadu_si128(with_offset(myy,i+1,offset)));
    const __m256i VM_0 = _mm256_mullo_epi16(VA_0, VB_0);
    const __m256i VM_1 = _mm256_mullo_epi16(VA_1, VB_1);
  ...
  if (m&1) {
    // deal  with the last iteration if m is odd
    const __m256i VA_0 = _mm256_set1_epi16(vx[mm1]);
    const __m256i VB_0 = _mm256_cvtepu8_epi16(_mm_loadu_si128(with_offset(myy,mm1,offset)));
    const __m256i VM_0 = _mm256_mullo_epi16(VA_0, VB_0);
...
void p251_vec_mat128cols_muladd_avx2_ct(void *vz, uint8_t const *vx,
  ...
  for (size_t i = 0; i < m; ++i) {
    ...
    for (size_t j = 0; j < 8; ++j) {
      const __m256i VB =
          _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i *)&myy[i * 8 * scaling + j]));
      const __m256i VM = _mm256_mullo_epi16(VA, VB);
      const __m256i VMLow = _mm256_unpacklo_epi16(VM, _mm256_setzero_si256());
      const __m256i VMHigh = _mm256_unpackhi_epi16(VM, _mm256_setzero_si256());
