...
/**
 * \fn void rbc_37_vec_set_random(samplable_t*, rbc_37_vec v, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_37_vec_set_random(samplable_t* ctx, rbc_37_vec v, uint32_t size) {
  ...
  byte_sample(ctx, random, size * bytes);
...
void rbc_37_vec_set_random_x4(samplable_x4_t* ctx, rbc_37_vec const v[4], uint32_t size) {
  uint32_t bytes = (RBC_37_FIELD_M % 8 == 0) ? RBC_37_FIELD_M / 8 : RBC_37_FIELD_M / 8 + 1;
  uint8_t random_mem[size * bytes * 4];
  uint8_t mask = (1 << RBC_37_FIELD_M % 8) - 1;
  uint8_t* random[4] = {
    &random_mem[size * bytes * 0], &random_mem[size * bytes * 1],
    &random_mem[size * bytes * 2], &random_mem[size * bytes * 3]
  };
  byte_sample_x4(ctx, random, size * bytes);

  for(uint32_t j=0; j<4; j++) {
    rbc_37_vec_set_zero(v[j], size);

    for(size_t i = 0 ; i < size ; ++i) {
      random[j][(i + 1) * bytes - 1] &= mask;
      memcpy(v[j][i], random[j] + i * bytes, bytes);
    }
  }
}
...
/**
 * \fn void rbc_37_vec_set_random_full_rank_with_one(samplable_t* ctx, rbc_37_vec o, uint32_t size) {
 ...
 * \param[in] size Size of the vector
 */
void rbc_37_vec_set_random_full_rank_with_one(samplable_t* ctx, rbc_37_vec o, uint32_t size) {
...
/**
 * \fn void rbc_37_vec_set_random_from_support(samplable_t* o, rbc_37_vec o, uint32_t size, const rbc_37_vec support, uint32_t support_size)
 ...
 */
void rbc_37_vec_set_random_from_support(samplable_t* ctx, rbc_37_vec o, uint32_t size, const rbc_37_vec support, uint32_t support_size, uint8_t copy_flag) {
  ...
  if(copy_flag) {
    ...
    byte_sample(ctx, random1, random1_size);
    ...
    byte_sample(ctx, random2, random2_size);
    ...
    byte_sample(ctx, random, random_size);
