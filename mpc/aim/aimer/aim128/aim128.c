...
// -----------------------------------------------------------------------------
/***************  MPC-in-the-Head Library  ***************
 *
 *  This file is part of the MPC-in-the-Head
 *  library (https://github.com/CryptoExperts/libmpcith).
 *  The current file has been slightly modified to be
 *  compatible with the library.
 * 
 *************** (END OF LIBMPCITH HEADER) ****************/
...
void generate_matrix_LU(const uint8_t* iv, GF** matrix_A, GF vector_b)
{
  ...
  for (size_t i = 0; i < NUM_INPUT_SBOX; i++)
  {
    ...
    {
      GF_transposed_matmul(temp_matrix[2 * i + 1][j], (const GF2_128*) temp_matrix[2 * i],
                           matrix_A[i][j]);
    }
...
void aim(const uint8_t* pt, const uint8_t* iv, uint8_t* ct)
{
  ...
  // linear layer
  GF_transposed_matmul(state[0], (const GF2_128*) matrix_A[1], state[0]);
  GF_transposed_matmul(state[0], (const GF2_128*) matrix_A[0], state[0]);

  GF_transposed_matmul(state[1], (const GF2_128*) matrix_A[3], state[1]);
  GF_transposed_matmul(state[1], (const GF2_128*) matrix_A[2], state[1]);
             ...
             const size_t num_parties, GF *z_shares, GF *x_shares)
{
  ...
  for (size_t party = 0; party < num_parties; party++)
  {
    ...
    // linear layer
    GF_transposed_matmul(z_shares[party_index], (const GF2_128*) matrix_A[0], state[0]);
    GF_transposed_matmul(z_shares[party_index + 1], (const GF2_128*) matrix_A[1], state[1]);
    if (party == 0)
    {
    ...
    GF_transposed_matmul(x_shares[party_index], (const GF2_128*) e2_power_matrix,
              ...
              GF* z_share, GF* x_share, char has_sharing_offset)
{
  ...
  // linear layer
  GF_transposed_matmul(t_share[0], (const GF2_128*) matrix_A[0], state[0]);
  GF_transposed_matmul(t_share[1], (const GF2_128*) matrix_A[1], state[1]);
  ...
  GF_transposed_matmul(x_share[0], (const GF2_128*) e2_power_matrix,
                        z_share[1]);
