...
void generate_matrix_LU(const uint8_t* iv, GF** matrix_A, GF vector_b)
{
  ...
  for (size_t i = 0; i < NUM_INPUT_SBOX; i++)
  {
    ...
    {
      GF_transposed_matmul(temp_matrix[2 * i + 1][j], (const GF2_256*) temp_matrix[2 * i],
                           matrix_A[i][j]);
    }
...
void aim(const uint8_t* pt, const uint8_t* iv, uint8_t* ct)
{
  ...
  // linear layer
  GF_transposed_matmul(state[0], (const GF2_256*) matrix_A[1], state[0]);
  GF_transposed_matmul(state[0], (const GF2_256*) matrix_A[0], state[0]);

  GF_transposed_matmul(state[1], (const GF2_256*) matrix_A[3], state[1]);
  GF_transposed_matmul(state[1], (const GF2_256*) matrix_A[2], state[1]);

  GF_transposed_matmul(state[2], (const GF2_256*) matrix_A[5], state[2]);
  GF_transposed_matmul(state[2], (const GF2_256*) matrix_A[4], state[2]);
             ...
             const size_t num_parties, GF *z_shares, GF *x_shares)
{
  ...
  for (size_t party = 0; party < num_parties; party++)
  {
    ...
    // linear layer
    GF_transposed_matmul(z_shares[party_index], (const GF2_256*) matrix_A[0], state[0]);
    GF_transposed_matmul(z_shares[party_index + 1], (const GF2_256*) matrix_A[1], state[1]);
    GF_transposed_matmul(z_shares[party_index + 2], (const GF2_256*) matrix_A[2], state[2]);
    ...
    GF_transposed_matmul(x_shares[party_index], (const GF2_256*) e2_power_matrix,
              ...
              GF* z_share, GF* x_share, char has_sharing_offset)
{
  ...
  // linear layer
  GF_transposed_matmul(t_share[0], (const GF2_256*) matrix_A[0], state[0]);
  GF_transposed_matmul(t_share[1], (const GF2_256*) matrix_A[1], state[1]);
  GF_transposed_matmul(t_share[2], (const GF2_256*) matrix_A[2], state[2]);
  if (has_sharing_offset)
  {
    GF_add(state[2], vector_b, state[2]);
  }
  GF_add(state[0], state[1], state[0]);
  GF_add(state[0], state[2], x_share[3]);

  // optimization B.1.2 in eprint 2022/588
  GF_copy(t_share[0], state[0]);
  GF_copy(t_share[1], state[1]);
  GF_copy(t_share[2], state[2]);

  GF_sqr(x_share[0], temp);
  GF_sqr(temp, temp);
  GF_sqr(temp, z_share[0]);

  GF_sqr(z_share[0], temp);
  GF_sqr(temp, temp);
  GF_sqr(temp, temp);
  GF_sqr(temp, z_share[2]);

  GF_transposed_matmul(x_share[0], (const GF2_256*) e2_power_matrix,
                        z_share[1]);

  GF_copy(state[0], x_share[0]);
  GF_copy(state[1], x_share[1]);
  GF_copy(state[2], x_share[2]);

  GF_sqr(x_share[3], temp);
  GF_sqr(temp, temp);
  GF_sqr(temp, temp);
  GF_sqr(temp, temp);
  GF_sqr(temp, temp);

  GF_mul(ct, x_share[3], state[0]);
  GF_add(temp, state[0], z_share[3]);
}
