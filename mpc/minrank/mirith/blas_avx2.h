...
/// \param in
/// \return
__m256i gf16_hadd_avx2_32_v2(const __m256i in) {
...
void gf16mat_new_core(uint8_t *__restrict c,
    ...
    //const __uint128_t cMask = column_A_bytes == 8 ? (-1ull) : ((((__uint128_t)(1)) << (8*column_A_bytes)) -1ul);
    __uint128_t ret = 0;
    // TODO
    const __m256i loadMask = _mm256_setr_epi32(-1,-1,-1,-1,-1,-1,-1,-1);
    ...
    Adata[nrA256-1] = _mm256_maskload_epi32((int *)(a + (nrA256 - 1)*maxByteInAVX2), loadMask);
    ...
    for (uint32_t k = 0; k < nr_cols_B; ++k) {
        ...
        //Bdata[nrA256-1] =_mm256_maskload_epi32((int *)(a + (nrA256 - 1)*32), )
        ...
        //if (k < nr_cols_B - 1) {
        //    *((__uint128_t *) (c + k * column_A_bytes)) ^= ret&cMask;
        //}
    ...
    //for (uint32_t i = 0; i < column_A_bytes; i++) {
    //    c[(nr_cols_B - 1)*column_A_bytes + i] ^= (uint8_t)(ret>>(i*8));
    //}
}
