...
//int matrix_bytes_per_column(int n_rows)
//{
//    return (n_rows >> 1) + (n_rows & 1);
//}

//int matrix_bytes_size2(int n_rows, int n_cols)
//{
//    return matrix_bytes_per_column(n_rows) * n_cols;
//}
...
void matrix_init_random(ff_t *matrix, int n_rows, int n_cols, samplable_t *prng)
{
    ...
    for (i = 0; i < n_cols; i++)
    {
        byte_sample(prng, matrix + i * matrix_height , matrix_height);
    }
...
void matrix_init_random_x4(ff_t * const* matrix, int n_rows, int n_cols, samplable_x4_t *prng)
{
    ...
    {
        ff_t* data[4] = {
            matrix[0] + i * matrix_height,
            matrix[1] + i * matrix_height,
            matrix[2] + i * matrix_height,
            matrix[3] + i * matrix_height,
        };
        byte_sample_x4(prng, data, matrix_height);
    }
    ...
    if (n_rows & 1)
    {
        for (uint32_t j = 0; j < 4; j++)
        {
            for (i = 0; i < n_cols; i++)
            {
                matrix[j][i * matrix_height + matrix_height_x ] &= 0x0f;
            }
        }
    }
}
...
void matrix_add(ff_t *matrix1, const ff_t *matrix2, int n_rows, int n_cols)
{
    ...
    if (rem)
    {
        temp_sumand1 = _load_ymm( (matrix1 + jump), rem );
        temp_sumand2 = _load_ymm( (matrix2 + jump), rem );
        temp_result = _mm256_xor_si256(temp_sumand1, temp_sumand2);
        _store_ymm(matrix1 +  jump, rem, temp_result);
    }
}
    ...
    int n_rows, int n_cols)
{
    ...
    n_bytes = matrix_bytes_size(n_rows, n_cols);
    // PATCH: "gf16v_madd_avx2" requires that
    //   "n_bytes" is a multiple of 4. So we work on copies
    //   with the right sizes, and we keep the desired part.
    ff_t* buffer1 = (ff_t*) calloc(((n_bytes+3)>>2)<<2, 1);
    ff_t* buffer2 = (ff_t*) calloc(((n_bytes+3)>>2)<<2, 1);
    memcpy(buffer1, matrix1, n_bytes);
    memcpy(buffer2, matrix2, n_bytes);
    gf16v_madd_avx2(buffer1, buffer2, scalar, n_bytes);
    memcpy(matrix1, buffer1, n_bytes);
    free(buffer1);
    free(buffer2);
}
...
{
    int n_bytes_per_column1;
    
    n_bytes_per_column1 = matrix_bytes_per_column(n_rows1);
