...
void uncompress_instance(instance_t* inst) {
    if(inst->A == NULL) {
        ...
        inst->A = (uint8_t(*)[PARAM_m][PARAM_MATRIX_BYTESIZE]) malloc(PARAM_m*PARAM_MATRIX_BYTESIZE);
        inst->b = (uint8_t(*)[PARAM_m][PARAM_n]) malloc(PARAM_m*PARAM_n);
        ...
        for(unsigned int i=0; i<PARAM_m; i++)
            vec_rnd((*inst->A)[i], (PARAM_n*(PARAM_n+1))/2, &entropy);
        vec_rnd((uint8_t*) *inst->b, PARAM_m*PARAM_n, &entropy);
    }
}
...
static void compute_the_mq_equations_outputs(
    ...
    for(int i=0; i<PARAM_m; i++) {
        ...
        for(uint32_t j=0; j<PARAM_n; j++) {
            tmp[j] = sca_add(tmp[j], sca_innerproduct(x+j, A[i]+ind, PARAM_n-j));
            ind += (PARAM_n-j);
        }
        // y_i = tmp^T x
        y[i] = 0;
...
void generate_template_instance(instance_t** tmpl, samplable_t* entropy) {
    *tmpl = (instance_t*) malloc(sizeof(instance_t));
    (*tmpl)->A = NULL;
    (*tmpl)->b = NULL;

    // Sample a seed
    byte_sample(entropy, (*tmpl)->seed, PARAM_SEED_SIZE);
}
...
void complete_template_instance(instance_t** inst, solution_t** sol, const instance_t* tmpl, samplable_t* entropy) {
    ...
    *inst = (instance_t*) malloc(sizeof(instance_t));
    ...
    // Sample a seed
    memcpy((*inst)->seed, tmpl->seed, PARAM_SEED_SIZE);
...
void generate_instance_with_solution(instance_t** inst, solution_t** sol, samplable_t* entropy) {
    instance_t* tmpl;
    generate_template_instance(&tmpl, entropy);
    complete_template_instance(inst, sol, tmpl, entropy);
    free_instance(tmpl);
}
