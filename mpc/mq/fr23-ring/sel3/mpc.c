...
#if !(PARAM_SELECTOR_DEGREE == 3 && PARAM_CIRCUIT_DEPTH == 3)
...
void mpc_emulate_party(vec_broadcast_t* broadcast, vec_challenge_t* mpc_challenge,
    ...
    for(uint32_t i=0; i<inter1_size; i++) {
        for(uint32_t j=0; j<info->selector_side_size[2]; j++) {
            uint32_t pos = (j+i*info->selector_side_size[2]);
            if(pos < info->ring_size)
                selector[pos] = gf251_mul(inter1[i], sel_vec[2][j]);
        }
    }
    free(inter1);
