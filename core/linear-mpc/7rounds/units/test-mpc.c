...
static int __mpc_test_plain_computation(samplable_t* entropy, char* last_error, int direct) {
    instance_t* inst;
    solution_t* wit;
    ...
    vec_challenge_1_t* challenge_1 = new_challenge_1();
    expand_mpc_challenge_hash_1(&challenge_1, digest, 1, inst);
    randombytes(digest, PARAM_DIGEST_SIZE);
    vec_challenge_2_t* challenge_2 = new_challenge_2();
    expand_mpc_challenge_hash_2(&challenge_2, digest, 1, inst);
    free(digest);
    
    vec_share_t* plain_input = new_share();
    vec_wit_set(get_wit(plain_input), wit);
    vec_unif_rnd(get_unif(plain_input), entropy);
    compute_hint(get_hint(plain_input), wit, get_unif(plain_input), inst, challenge_1);

    vec_broadcast_t* plain_br = new_br();
    if(direct)
        mpc_compute_plain_broadcast(plain_br, challenge_1, challenge_2, plain_input, inst);
    else
        mpc_compute_communications(plain_br, challenge_1, challenge_2, plain_input, plain_br, inst, 1);
    free_instance(inst);
...
static int __mpc_test_multiparty_computation(samplable_t* entropy, char* last_error, int with_random_shares) {
    instance_t* inst;
    solution_t* wit;
    // Generation instance with solution
    generate_instance_with_solution(&inst, &wit, entropy);
    ...
    for(uint16_t i=0; i<nb_parties; i++) {
        mpc_compute_communications(broadcast, challenge_1, challenge_2, shares[i], plain_br, inst, (i==nb_parties-1));
...
static int __mpc_test_serialization_plain_broadcast(samplable_t* entropy, char* last_error, int num_test) {
    instance_t* inst;
    solution_t* wit;
    // Generation instance with solution
    generate_instance_with_solution(&inst, &wit, entropy);
