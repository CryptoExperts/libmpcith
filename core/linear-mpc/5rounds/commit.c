...
void commit_seed_and_aux(uint8_t* digest, const uint8_t* seed, const vec_wit_t* wit, const vec_corr_t* corr, const uint8_t* salt, uint16_t e, uint16_t i) {
    hash_context ctx;
    hash_init_prefix(&ctx, HASH_PREFIX_COMMITMENT);
    hash_update(&ctx, salt, PARAM_SALT_SIZE);
    hash_update_uint16_le(&ctx, e);
...
void commit_share_x4(uint8_t** digest,  vec_share_t const* const* share, const uint8_t* salt, uint16_t e, const uint16_t* i) {
    commit_string_x4(digest, (uint8_t const* const*) share, sizeof(vec_share_t), salt, e, i);
}
...
void commit_seed_and_aux_x4(uint8_t** digest, uint8_t const* const* seed, vec_wit_t const* const* wit, vec_corr_t const* const* corr, const uint8_t* salt, uint16_t e, const uint16_t* i) {
    hash_context_x4 ctx;
    hash_init_prefix_x4(&ctx, HASH_PREFIX_COMMITMENT);
    hash_update_x4_1(&ctx, salt, PARAM_SALT_SIZE);
    hash_update_x4_uint16_le(&ctx, e);
