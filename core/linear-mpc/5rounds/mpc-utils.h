...
static inline vec_corr_t* new_corr(void) {
    return (vec_corr_t*) malloc(PARAM_CORR_SIZE);
}
...
static inline vec_challenge_t* new_challenge(void) {
    return (vec_challenge_t*) malloc(sizeof(vec_challenge_t));
}
...
static inline vec_corr_t* get_corr(vec_share_t* sh) {
    return (vec_corr_t*) (((uint8_t*)sh) + PARAM_WIT_SIZE + PARAM_UNIF_SIZE);
}
...
static inline const vec_corr_t* get_corr_const(const vec_share_t* sh) {
    return (const vec_corr_t*) (((uint8_t*)sh) + PARAM_WIT_SIZE + PARAM_UNIF_SIZE);
}
...
build_all_arithmetic_operations(corr, vec_corr_t)
...
build_all_compression_operations(corr, vec_corr_t)
