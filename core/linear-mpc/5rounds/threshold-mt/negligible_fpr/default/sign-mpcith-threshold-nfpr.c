...
static void hash_for_view_challenge(uint8_t challenge_hash[PARAM_DIGEST_SIZE], const uint8_t mpc_challenge_hash[PARAM_DIGEST_SIZE],
    const vec_broadcast_t* broadcast[PARAM_NB_EXECUTIONS][PARAM_NB_REVEALED], const vec_broadcast_t* plain_broadcast, 
...
int mpcith_threshold_nfpr_sign(uint8_t* sig, size_t* siglen,
    ...
    compute_correlated(get_corr(plain), plain_wit, get_unif(plain), ssk.inst);
    ...
    hash_for_view_challenge(challenge_views_hash, ssig->mpc_challenge_hash, (const vec_broadcast_t*(*)[PARAM_NB_REVEALED]) broadcast, ssig->plain_broadcast, ssig->salt, m, mlen);
    ...
    for(e=0; e<PARAM_NB_EXECUTIONS; e++) {
        ...
        for(p=0; p<PARAM_NB_REVEALED; p++)
            compute_share_wit(ssig->proofs[e].wit[p], plain, (vec_share_t const**) rnd[e], (uint8_t) open_views[e][p]);
    }
